...
class SmartCountdownGoogleCal_Helper {
	...
	public static function apiKeyUploadControl( $name, $is_uploaded ) {
		$html = array();
		...
		$html[] = '<input type="file" class="upload-p12-file" id="' . $name . '" name="' . $name . '" value=""' . $input_style . ' />';
		return implode( "\n", $html );
	}
	public static function getEvents( $instance, $configs ) {
		...
		foreach( $configs as $preset_index => $config ) {
			...
			if( !empty( $gcal_cache ) ) {
				...
				$cache_time = ( int ) get_option( SmartCountdownGoogleCal_Plugin::$option_prefix . 'events_cache_time_' . $preset_index );
			...
			if( is_null( $events ) ) {
				...
				if( $auth_mode == 'api_key' ) {
					$api_key = get_option( SmartCountdownGoogleCal_Plugin::$option_prefix . 'api_key_' . $preset_index, '' );
					...
					$auth_email = get_option( SmartCountdownGoogleCal_Plugin::$option_prefix . 'auth_email_' . $preset_index, '' );
					$p12key = get_option( SmartCountdownGoogleCal_Plugin::$option_prefix . 'api_p12key_' . $preset_index, '' );
