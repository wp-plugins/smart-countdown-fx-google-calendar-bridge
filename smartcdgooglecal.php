...
<?php
	...
	private static function registerSettings( $preset_index ) {
		...
		//reserved to replace auth_key_file, if activated as is causes wrong recursion, looks that update_option calls validation...
		//register_setting( self::$options_page_slug, self::$option_prefix . 'api_p12key_' . $preset_index, 'SmartCountdownGoogleCal_Plugin::uploadKeyFile' );
...
function smartcountdown_google_cal_uninstall() {
	foreach( array( 1, 2 ) as $preset_index ) {
		...
		// delete options
		// stand-alone
		delete_option( self::$options_page_slug, self::$option_prefix . 'events_cache_' . $preset_index );
		delete_option( self::$options_page_slug, self::$option_prefix . 'auth_key_file_' . $preset_index );
